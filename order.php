...
where status= '1' and user.name='$name' ";
    ...
    <div class="flex-fill bd-highlight p-0">
        <div class=" bg-light p-4" style="min-height:100vh ">
            <form action="insert-order.php" method="get">
                ...
                $sql = "SELECT name,id FROM product";
                $result = mysqli_query($conn, $sql);
                while ($row = mysqli_fetch_array($result)) {
                    echo '<option name="product_name" >' . $row['name']  . '</option>';
                }
                ...
                <div class="mb-2">
                    ...
                    <input type="date" name="date_start" class="form-control" id="date_start" value="<?= $value["date_start"] ?>">
                </div>
                <div class="mb-2">
                    <label for="date">結束日期:</label>
                    <input type="date" name="date_end" class="form-control" id="date_end" value="<?= $value["date_end"] ?>">
                </div>
                <div class="mb-2">
                    <!-- <label for="">總金額: </label>
