...
$sql="UPDATE product SET name=?, price=?, number=?, viewpoint_id=?, itinerary=?, guide_id=?, content=?, status=? WHERE id= ?";
...
try{
    $stmt->execute([$name, $price,$number,$viewpoint_id_str,$itinerary,$guide_id,$content,$status, $id]);
    echo "資料修改完成";
