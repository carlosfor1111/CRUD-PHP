        ...
        <div class=" bg-light p-4" style="min-height:100vh ">
            <form action="php/update_product.php" method="post">
                ...
                <div class="mb-2">
                    ...
                    <?php
                        $a=$rows["guide_id"];                        
                        $stmt_guide=$db_host->prepare("SELECT id,name FROM guide WHERE id=$a");
                        $stmt_guide->execute();
                ...
                <div class="mb-2">
                    <label>行程介紹：</label>
                    <textarea name="content" class="form-control"  id="floatingTextarea2" style="height: 300px"><?= $rows["content"]?></textarea>
                </div>
                <?php 
                    switch($rows["status"]){
                        case '0':
                ...
                <a class="btn btn-info" href="product_list.php">返回列表</a>
                <?php 
                } 
                endif?>
            </form>
