...
$start_item=($p-1)*10; //起始的商品=頁數-１在乘上10
    ...
    $sql="SELECT viewpoint.*,area.name AS area_name , topic.name AS  topic_name FROM  viewpoint
      ...
      WHERE  viewpoint.name LIKE '%$search%' 
...
JOIN  area  ON area.id = viewpoint.area_id 
JOIN  topic ON topic.id= viewpoint.topic_id
WHERE area.name LIKE '%$search%' OR viewpoint.name LIKE '%$search%' OR topic.name LIKE '%$search%' 
...
<div class="d-flex m-0">
    ...
    <div class="flex-fill bg-light p-0">
        <div class=" p-4" style="min-height:100vh ">
            ...
            <table 
            data-toggle="table"  
            class="table table-bordered my-3 ml-2 align-middle text-center" 
            style="width:100%"
            >
